...
exports.userRouter
    ...
    const { username, password, email, first_name, last_name, country, is_native, type, intro, role } = req.body;
    let user = yield req.userRepository.findOne({ $or: [{ username }, { email }] });
    ...
    return res.status(200).send(jwtGenerator_1.generateJwt(user));
}))
    ...
    const { email, first_name, last_name, country, is_native, type, intro } = req.body;
